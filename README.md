...
- **Smart PDF Selection**: Automatically scans the root directory for `.pdf` files. If multiple files are found, it provides an interactive selection menu.
...
- **Zero-Configuration Prompting**: If your `NVIDIA_API_KEY` is not set, the script will securely prompt you for it on the first run and offer to save it in `.env` (which is safely git-ignored).
...
graph TD
    ...
    D -->|Top K Context| H[ChatPromptTemplate]
    F --> H
...
4. **Retrieval & RAG Chain**: When you ask a question, the vector store retrieves the top 4 most similar chunks. These chunks are embedded into a system prompt context and sent to `meta/llama-3.1-8b-instruct` to formulate a concise, factual response.

---

## 🚀 Future Enhancements (Roadmap)

- [ ] **Multi-PDF Directory Support**: Expand the loader to use `DirectoryLoader` to scan and index a directory of multiple PDFs.
- [ ] **Interactive Web UI**: Wrap the chatbot in a beautiful frontend using **Streamlit** or **Gadio** to allow drag-and-drop PDF uploads.
